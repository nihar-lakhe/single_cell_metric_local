...
# ============================================================
# Hierarchical Attention (HA) aggregator — Do et al. style
# ============================================================
#
# Strict port of the `HAModel` used in `ha_20seeds_3fold_cv.ipynb`.
#
#   Level 1 (attn1): attention over cells *within* each cell-type group
#                    (torch_geometric softmax + global_add_pool).
#   Level 2 (attn2): attention over the per-cell-type representations
#                    *within* a patient (plain softmax + weighted sum).
#
# Unlike the notebook — which packs many patients into one flat batch using
# group indices `idx * n_ct + ct_dict[ct]` — the framework processes **one
# patient at a time**. The per-cell group index is therefore simply the
# cell-type index in `[0, n_ct)`, and the pooling `size` is `n_ct`.
#
# Cell types whose cells are absent for a patient produce an all-zero row
# after `global_add_pool` (size=n_ct); those zero rows still participate in
# the level-2 softmax exactly as they do in the notebook, so behaviour is
# preserved.
...
# ============================================================
# AggregatorPlusClassifier — the complete HA head
# passed to SingleCellMetricModel as aggregator_plus_classifier
# ============================================================

class AggregatorPlusClassifier(nn.Module):
    """
    Hierarchical Attention aggregator + classifier.

    Wraps `HAModel` so it conforms to the framework aggregator contract:
    a single forward that maps a patient's cell embeddings to class logits.
    ...
    Args:
        ...
        attn1        : bool  — cell-level attention
        attn2        : bool  — cell-type-level attention
        n_hid        : int   — hidden dimension
        ...
        normalize    : bool  — L2-normalise cell embeddings before pooling.
                               ...
                               already L2-normalises cell embeddings, and the HA
...
def build_aggregator(
    ...
    """
    Build the Hierarchical Attention aggregator + classifier head.
...
# ============================================================
# SingleCellMetricModel — scGPT backbone + LoRA + HA aggregator
# ============================================================
...
class SingleCellMetricModel(nn.Module):
    ...
    def forward(self, batch: Dict[str, torch.Tensor], chunk_size: int = 512):
        ...
        # Aggregate + classify via the externally-supplied HA head
